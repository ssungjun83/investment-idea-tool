...
/**
 * GET /api/crushed/cron — Vercel Cron Job (평일 22:30 UTC = KST 07:30)
 */
  ...
  try {
    ...
    for (const sectorDef of SECTOR_WATCHLIST) {
      try {
        ...
        console.error(`[cron-crushed] ${sectorDef.sector} error:`, err);
      }
    }
    ...
    return NextResponse.json({ date: today, sectors: sectors.length, stocks: totalStocks });
  } catch (err) {
    console.error("[cron-crushed] error:", err);
